...
def train_dgl(
    ...
    for e in range(epochs):
        ...
        for dats in train_loader:
            ...
            # print ('dats=',dats[2])
            optimizer.zero_grad()
            output = net([dats[0].to(device), dats[1].to(device)])
            loss = criterion(output[0], dats[2].to(device))
            loss.backward()
            optimizer.step()
    ...
    if "atomwise" not in config.model.name:
        ...
        def log_results(engine):
            ...
            if config.progress:
                ...
                if not classification:
                    ...
                    pbar.log_message(
                        f"Train ROC AUC: {tmetrics['rocauc']:.4f}"
                    )
    ...
    if (
        ...
        if (
            config.store_outputs
            and not classification
            and "atomwise" not in config.model.name
        ):
            x = []
            y = []
