...
class ALIGNNAtomWiseTorchConfig(BaseSettings):
    """Hyperparameter schema for jarvisdgl.models.alignn."""

    name: Literal["alignn_atomwise_torch"]
    alignn_layers: int = 4
    gcn_layers: int = 4
...
class ALIGNNAtomWiseTorch(nn.Module):
    ...
    def __init__(
        self,
        config: ALIGNNAtomWiseTorchConfig = ALIGNNAtomWiseTorchConfig(
            name="alignn_atomwise_torch"
        ),
    ):
    ...
    def forward(
        ...
        # g = g.local_var()
        result = {}
        lattice_mat = g.gdata.unfold(0, 3, 3).mT  # ['lattice_mat'][0]
        ...
        vol = torch.sum(
            torch.tensor(
                [
                    torch.abs(torch.matmul(torch.cross(p[0], p[1]), p[2]))
                    for p in lattice_mat
                ]
            )
        )
        ...
        if self.config.calculate_gradient:
            ...
            X_src2 = cart_coords[src]
            r = (X_src2 - X_dst).float()
            # g2 = Graph.atom_dgl_multigraph_torch(
            #    cart_coords=cart_coords,
            #    frac_coords=frac_coords,
            #    lattice_mat=lattice_mat,
            # )
            # r = g2.edata["r"]
            # lg2 = g2.line_graph(shared=True)
            # lg2.apply_edges(compute_bond_cosines)
            # z = self.angle_embedding(lg.edata.pop("h"))
        else:
            r = g.edata["r"]
        ...
        if self.config.calculate_gradient:
            ...
            if self.config.stresswise_weight != 0:
                ...
                stress = (
                    -1
                    * (160.21766208 * torch.matmul(cart_coords.T, gradient))
                    / vol
                )
