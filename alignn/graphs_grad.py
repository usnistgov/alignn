...
class StructureDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.ids = [i[id_tag] for i in self.dataset]  # self.df[id_tag]
        self.labels = defaultdict()
        for ii, i in enumerate(self.dataset):
            self.labels[ii] = {}
        for ii, i in enumerate(self.dataset):
            self.labels[ii][self.target] = torch.tensor(i[self.target]).type(
                torch.get_default_dtype()
            )

            if "atomwise_target" in dataset[0]:
                self.atomwise = True
                self.labels[ii][self.target_atomwise] = torch.tensor(
                    i[self.target_atomwise]
                )
                # self.labels[ii][self.target_atomwise]=torch.tensor(np.array(i[self.target_atomwise]))
        """
        self.labels[target]=torch.tensor(np.array([i[self.target] for i in self.dataset])).type(
            torch.get_default_dtype()
        )
        if 'atomwise_target' in dataset[0]:
         self.atomwise=True
         self.atomwise_labels=[]
         for i in dataset:
           #for j in i[target_atomwise]:
                 self.atomwise_labels.append( np.array([i[target_atomwise]]))
                 #self.atomwise_labels.append(j)
         self.labels[target_atomwise]=self.atomwise_labels.

         print ('self.dataset',(self.atomwise_labels))
         #self.atomwise_labels =torch.tensor(np.array(self.atomwise_labels,dtype='float')).type(
         #self.atomwise_labels =torch.tensor(self.atomwise_labels).type(
         #   torch.get_default_dtype()
         #)
         print('self.atomwise_labels',self.atomwise_labels)
        """
    ...
    def __len__(self):
        """Get length."""
        return len(list(self.labels.values()))  # shape[0]

    def __getitem__(self, idx):
        """Get StructureDataset sample."""
        g = self.graphs[idx]
        # label = self.labels[idx][self.target]
        label = self.labels[idx][self.target_atomwise]
        if self.transform:
            g = self.transform(g)
    ...
    def collate_line_graph(
        ...
        print("labelsss", lbl)
        """
        labels=lbl #torch.stack(lbl)
        """
        print("labels =", labels)
        # print ('graphs =',graphs,graphs[0].number_of_nodes())
